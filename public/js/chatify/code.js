  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        $(".messenger-infoView")
          .find(".avatar")
          .css("background-image", 'url("' + data.fetch.avatar.replace("storage/", "") + '")');
        $(".header-avatar").css(
          "background-image",
          'url("' + data.fetch.avatar.replace("storage/", "") + '")'
        );
        ...
        messageInput.focus();
        // update info in view
        $(".messenger-infoView .info-name").text(data.fetch.user_name);
        $(".m-header-messaging .user-name").text(data.fetch.user_name);
        // Star status
        data.favorite > 0
